...
export async function ingestLog({ founder, content, projectId, userId }) {
  ...
  const otherTone = previousOtherLog ? classifyTone(previousOtherLog.content) : "neutral";
  const conflictingTone =
    previousOtherLog && currentTone !== "neutral" && otherTone !== "neutral" && currentTone !== otherTone;

  const conflictSignals = ["delay", "blocked", "change", "cancel", "postpone", "revise", "hold", "risk", "not", "can't", "cannot"];
  const signalHit = conflictSignals.some((word) => content.toLowerCase().includes(word));

  if (previousOtherLog && (conflictingTone || signalHit)) {
    state.conflicts.push(makeConflict({ founder, content, otherLog: previousOtherLog, projectId, currentLogId: log.id }));
  }

  await writeState(state);
  return { log, createdConflict: Boolean(previousOtherLog && (conflictingTone || signalHit)) };
}

export async function listProjectState(projectId) {
  const state = await readState();
  const logs = sortNewestFirst(state.logs.filter((log) => log.projectId === projectId && log.status !== "archived"));
