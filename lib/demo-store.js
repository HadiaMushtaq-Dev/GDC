...
const RUNTIME_DATA_FILE = path.join("/tmp", "gdc-state.json");
...
let stateFilePath = IS_PROD ? RUNTIME_DATA_FILE : DATA_FILE;
...
async function ensureStateFile() {
  try {
    ...
    try {
      await fs.writeFile(stateFilePath, JSON.stringify(EMPTY_STATE, null, 2), "utf8");
    } catch (e) {
      if (e?.code === "EROFS" || e?.code === "EACCES") {
        stateFilePath = RUNTIME_DATA_FILE;
        await fs.mkdir(path.dirname(stateFilePath), { recursive: true });
        try {
          await fs.writeFile(stateFilePath, JSON.stringify(EMPTY_STATE, null, 2), "utf8");
        } catch {}
...
async function seedFirestoreFromState() {
  if (!seedPromise) {
    seedPromise = (async () => {
      try {
        const [projectsSnapshot, logsSnapshot, conflictsSnapshot, usersSnapshot] = await Promise.all([
          getDocs(firestoreCollection("projects")),
          getDocs(firestoreCollection("logs")),
          getDocs(firestoreCollection("conflicts")),
          getDocs(firestoreCollection("users")),
        ]);

        if (!projectsSnapshot.empty || !logsSnapshot.empty || !conflictsSnapshot.empty || !usersSnapshot.empty) {
          return;
        }
        ...
        const seeds = [
          ...
          ...state.users.map((item) => ({ collectionName: "users", item })),
        ];
        ...
        for (const [token, userId] of Object.entries(state.sessions || {})) {
          await setCollectionDoc("sessions", { id: token, userId });
        }
      } catch (err) {
...
export async function registerUser({ email, password, name, organization, role }) {
  ...
  await setCollectionDoc("users", user);
...
export async function createSession(userId) {
  ...
  await setCollectionDoc("sessions", { id: token, userId, createdAt: new Date().toISOString() });
  return token;
}

export async function removeSession(token) {
  if (isProdDemoAuth() || !token) {
    return;
  }

  await seedFirestoreFromState();
  await deleteCollectionDoc("sessions", token);
}
...
export async function getUserFromSession(token) {
  ...
  await seedFirestoreFromState();
  const sessionSnapshot = await getDoc(doc(db, "sessions", token));
  if (!sessionSnapshot.exists()) return null;

  const session = sessionSnapshot.data();
  if (!session?.userId) return null;

  const userSnapshot = await getDoc(doc(db, "users", session.userId));
  if (!userSnapshot.exists()) return null;

  return sanitizeUser({ id: userSnapshot.id, ...userSnapshot.data() });
}
